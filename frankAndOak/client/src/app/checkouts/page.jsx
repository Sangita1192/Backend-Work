...
export default function Checkout() {
  ...
  const [shipCharge, setShipCharge] = useState(null);
  ...
  useEffect(() => {
    if (user.data) setUserDetail(user.data);
  }, [user])
  ...
  useEffect(() => {
    ...
    if (cartData.data) {
      ...
      setTotalAmt(total);
      setCartItem(cartData.data.length);
    }
    console.log(cart);
  }, [cartData])
  ...
  const handleCheckout = (e) => {
    ...
    .then((response) => {
        //front end stripe key and it return promise
        if (response.data.session) {
          ...
          console.error('Session ID missing from server response');
        }
      })
    .catch ((error) => {
    console.log(error);
  ...
  <>
    ...
    <section className="grid lg:grid-cols-[55%_auto] grid-cols-1 mx-auto font-Poppins">
      <div className="h-screen flex justify-end lg:order-1 order-2">
        <form method='post' className="lg:w-[75%] w-full px-5 py-10 overflow-y-scroll h-screen scrollbar-hide" onSubmit={handleCheckout}>
          <div>
            ...
            <div className=" flex justify-center py-3">
              ...
              <button
                type="button"
                className="text-white bg-[#2557D6] hover:bg-[#2557D6]/90 focus:ring-4 focus:ring-[#2557D6]/50 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center me-2 mb-2"
              >
          ...
          <div className="space-y-4 py-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <select className="w-full rounded-md py-3 text-sm border-customBorder focus:ring-customBorder"
                name="state"
                id="state">
        ...
        <div className="lg:w-[75%] w-full">
          <div className="space-y-3">
            {
              cart.map((item, index) => (
                <div className="flex items-center gap-3" key={index}>
                  <figure className="relative border bg-[#E8F0EE] border-customBorder rounded-md px-2 w-[80px]">
                    <img
                      src={filePath + item.product.thumbnail}
                      alt=""
                      className="w-100"
          ...
          <div className="border-t border-[#E3E3E3] py-5 mt-5">
            ...
            <div className="pt-5 space-y-3">
              <div className="flex justify-between ">
                <div className="text-sm font-normal">Subtotal • {cartItem} items</div>
