      ...
      <form className='relative lg:w-[42%] md:w-[80%] h-[700px] overflow-scroll  px-10 pt-5 pb-8 bg-[#F9F9F9] overflow-x-hidden mt-5'>
        ...
        <div className="flex justify-between py-8 px-6">
          <div className="text-center space-y-1.5">
            <svg className="mx-auto" xmlns="http://www.w3.org/2000/svg" width="23" height="24" viewBox="0 0 23 24" fill="none"><g clipPath="url(#clip0_2406_20469)">
          ...
          <div className="text-center space-y-1.5">
            <svg className="mx-auto" xmlns="http://www.w3.org/2000/svg" width="23" height="23" viewBox="0 0 23 23" fill="none"><g clipPath="url(#clip0_2406_20485)"><path fillRule="evenodd" clipRule="evenodd" d="M0.0208 8.73C-0.0492 9.1 0.0608 9.49 0.3008 9.78L10.8608 22.31C11.3108 22.85 12.1208 22.92 12.6608 22.46L21.7708 14.78C22.3108 14.33 22.3808 13.52 21.9208 12.98L11.3608 0.45C11.1108 0.16 10.7508 0 10.3808 0L2.3008 0.03C1.8808 0.03 1.5208 0.33 1.4508 0.75L0.0208 8.73ZM4.8308 3.93C5.2208 3.58 5.8508 3.61 6.2108 4.05C6.5308 4.44 6.4908 5.07 6.0908 5.43C5.6608 5.75 5.0708 5.71 4.7108 5.31C4.3608 4.88 4.3908 4.29 4.8308 3.93Z" fill="black"></path></g><defs><clipPath id="clip0_2406_20485"><rect width="22.21" height="22.77" fill="white"></rect></clipPath></defs></svg>
...
function SignUpBox({ setCompStatus, compStatus }) {
  ...
  const [otpText, setOtpText] = useState("Generate OTP");
  ...
  const handleGenerateOtp = () => {
    ...
    const otpInterval = setInterval(() => {
      if (count > 1) {
        count--;
        setIfOtp(true);
        setOtpText(`Generate OTP in ${count}`);

      } else {
        clearInterval(otpInterval);
        setOtpText("Generate OTP");
        setIfOtp(false);
      }
    }, 1000)
    axios.post('http://localhost:4800/api/website/users/generate-otp', data)
      .then((response) => {
  ...
  const RegisterUser = () => {
    axios.post('http://localhost:4800/api/website/users/register-user', data)
      .then((response) => {
        console.log(response.data);
        Cookies.set('frankandoak_user', response.data.auth);
      ...
      .catch((error) => {
        ...
        // let str = error.response.data.errors[0];
        // if (str.includes("password")) {
        //   alert(error);
        //   Swal.fire({
        //     icon: "error",
        //     title: "Invalid Credential...",
        //     text: "Password must contains atleast 8 characters",  
        //   });
        // }
      })
  }
      ...
      <form action="post" >
        <div className="py-5 border-t border-gray-300">
          <div className="grid grid-cols-2 gap-5 mb-3">
            <input
              className="p-3 border text-[#757575] text-[14px] font-semibold border-[#757575] " type="text"
          ...
          <div className="flex flex-col gap-3">
            ...
            />
            <input
              className="p-3 border text-[#757575] text-[14px] font-semibold border-[#757575] " type="password"
              placeholder="Password"
              id="password"
              name="password"
            ...
            <div>
              ...
              <button
                className={`${(ifOtp) ? 'block' : 'hidden'}  p-3.5 mt-4 w-full bg-black text-white font-semibold`}
                type="button"
